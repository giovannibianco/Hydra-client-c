...
void print_usage_and_die(FILE * out){
    ...
    fprintf (out, "  -m <mode>        : the permission to use for the new file (default is 0640).\n");
...
int main(int argc, char* argv[]) {
    ...
    int flag;
    while ((flag = getopt (argc, argv, "m:qhv")) != -1) {
        switch (flag) {
            case 'm':
    ...
    if ((mode < 0001) || (mode > 0777)) {
        TRACE_ERR((stderr,"Invalid permission specified. The value should be in the range [0001,0777]\n"));
        print_usage_and_die(stderr);
    }
    ...
    // Get The Logger
    // -------------------------------------------------------------------------
    Category& logger = Category::getInstance("glite-io-client-put");
    ...
    while (bytesread < size) {
        ...
        if (nread <= 0) {
            const char * error_msg = strerror(errno);
            TRACE_ERR((stderr,"\nFatal error during local read. Error is \"%s (code: %d)\"\n",error_msg, errno));
	...
	int enc_buffer_size;
	char *enc_buffer;
	if (glite_eds_encrypt_block(ectx, buffer, nread, &enc_buffer, &enc_buffer_size, &error))
	{
